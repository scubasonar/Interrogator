...
namespace DSS.Devices
{
    public class uOLED
    {
        ...
        public uOLED(SerialPort p)
        {
            ...
            if(dPort.IsOpen)
                AutoBaud();
            dPort.Write(new byte[] { 0x59, 0x04, 0xFF }, 0, 3);
            response = Ack();
            int k = 0;
        }
        ...
        */
        ...
        {
            dPort.Write(new byte[] { GSGC_RECTANGLE, x1, y1, x2, y2, color[0], color[1] }, 0, 7);
            return Ack();
        }

        // pen size = 0 = solid
        // pen size = 1 = wireframe
        public bool DrawRectangle(byte x1m, byte x1l, byte y1m, byte y1l, byte x2m, byte x2l, byte y2m, byte y2l, byte[] color)
        {
            dPort.Write(new byte[] { GSGC_RECTANGLE, x1m, x1l,  y1m, y1l, x2m, x2l, y2m, y2l, color[0], color[1] }, 0, 11);
            return Ack();
        }
        ...
        {
            dPort.Write(new byte[] { GSGC_IMAGE, 0, x, 0, y, 0, width, 0, height, colorMode }, 0, 10);
